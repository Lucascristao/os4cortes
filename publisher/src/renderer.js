...
for (const [network, label] of Object.entries({ tiktok: 'TikTok', instagram: 'Instagram', youtube: 'YouTube' })) {
  ...
  document.querySelector('#accounts').append(row);
}
async function refresh() {
  try {
    const s = await window.os4.state();
    document.querySelector('#phase').textContent = s.phase;
    ...
    for (const n of ['tiktok', 'instagram', 'youtube']) {
      ...
      document.getElementById(`status-${n}`).textContent = `${s.accounts[n]?.status || 'Conta ainda não informada'}${s.open.includes(n) ? ' · navegador aberto (faça login e feche quando concluir)' : ''}`;
    }
  } catch (e) {
    message.textContent = e.message;
  }
